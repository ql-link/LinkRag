...
功能: Wiki 标题树
  ...
  场景: 搜索最终水合丢弃少量失效候选并从有界池补位
    假如 当前页 BM25 候选 C1 到 C7 已按顺序产生
    并且 最终水合前 C1 和 C2 已不属于当前 Chunk 真值
    当 用户执行页大小为 5 的 Wiki 搜索
    那么 当前页按原顺序返回 C3 到 C7
    并且 C1 和 C2 不出现在 results 或 chunks 中
    并且 补位不扩大每知识库固定候选深度

  场景: 搜索有界候选全部失效时返回 200 空页
    假如 当前有界 BM25 候选在最终水合前全部失效
    当 用户执行 Wiki 搜索
    那么 HTTP 响应状态为 200
    并且 results 和 chunks 均为空
    并且 has_more 为 false

  场景: 标题原预览失效时按当前可见引用重算摘要
    假如 标题 H1 的原首个直属 Chunk C1 已失效
    并且 当前首个可见直属 Chunk 为 C2 且直属总数为 2
    当 用户搜索命中标题 H1
    那么 H1 保持返回且预览改为 C2
    并且 H1 的直属总数、has_more 和展开游标基于当前可见引用
    并且 C1 不出现在响应中

  场景: 显式 Chunk 定位包含失效 ID 时整体拒绝
    假如 客户端批量定位有效 Chunk C1 和失效 Chunk C2
    当 用户调用 Chunk 定位接口
    那么 HTTP 响应状态为 403
    并且 响应不泄露 C1 或 C2 的存在性与标题路径

  场景: 标题展开跳过失效直属 Chunk 并从后项补位
    假如 标题 H1 的展开候选依次为 C1、C2 和 C3
    并且 最终水合前 C1 已失效而标题 H1 仍可见
    当 用户以页大小 2 展开标题 H1
    那么 当前页按原顺序返回 C2 和 C3
    并且 C1 不占页面名额
    当 标题 H1 本身失效或越权
    那么 HTTP 响应状态为 403

  场景: 宽松候选水合的数据库异常仍然失败
    假如 服务端内部候选执行最终可见性 SQL 时连接失败
    当 用户执行 Wiki 搜索
    那么 HTTP 响应状态为 500
    并且 数据库异常不被解释为空结果或失效候选

  场景: 旧 Chunk 失效后不猜测映射到相似的新 Chunk
    假如 旧 Chunk C1 已失效且新 Chunk C2 正文与其相似
    并且 当前检索候选只包含 C1
    当 用户执行 Wiki 搜索
    那么 C1 和 C2 均不出现在响应中
    并且 只有 C2 自身进入当前候选时才允许返回

  场景: 连续失效候选只在固定有界窗口内扫描
    假如 当前有界 BM25 候选 C1 到 C5 中仅 C5 仍可见
    当 用户执行页大小为 3 的 Wiki 搜索
    那么 当前页只返回 C5 且允许短页
    并且 可见性检查只扫描 C1 到 C5
    并且 正常候选失效不返回 409 或 500
