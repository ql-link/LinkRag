...
# 范围：Python 侧对外 RAG 问答流 SSE 端点。承接完整 RAG 行为：access JWT 鉴权 → 召回 → 候选融合 →
...
Feature: 对外 RAG 问答流 SSE
  ...
  Background:
    ...
    And 配置单用户最大并发召回流数 RAG_MAX_CONCURRENT_PER_USER=3
    And Redis 可用用于并发流计数
  ...
  Scenario: 模型可用且命中且正文可回填时以 answer_delta/answer_done 返回生成答案
    Given Java access token 对应用户 sub=123 aud=tolink-rag-frontend iss=tolink-java dataset_ids=[1,2] 有效
    And config_id 指向的 CHAT 模型对用户 123 可用
    And bm25 与 sparse 两路均返回命中
  ...
  Scenario: token 仅约束建连，建连后流执行期间 token 生命周期变化不中断流
    Given Java access token 对应用户 sub=123 dataset_ids=[1] 合法
    And config_id 指向的 CHAT 模型对用户 123 可用
    And 已用该 token 成功建连且召回正在执行
    When Java 端管理的 token 有效期在流执行期间到达
    Then 当前 SSE 流不因 token 生命周期变化被中断
  ...
  # ==== access token 鉴权（密码学失败矩阵由 python_standalone_auth.feature 覆盖）====
  ...
  # ==== access token 在 Java 管理的有效期内可复用 ====

  Scenario: 有效 token 可在有效期内重复建连（断线重连复用 access token）
    Given Java access token 对应用户 sub=123 dataset_ids=[1] 有效
    And config_id 指向的 CHAT 模型对用户 123 可用
    And 已用该 token 成功建连过一次且连接已结束
    When 前端在 token 有效时携带同一 access token 再次调用 POST /api/v1/rag/stream body query="任意" dataset_ids=[1]
    Then HTTP 响应状态为 200
    And 最终收到 SSE 事件 "answer_done"

  # ==== 身份与授权范围（身份取自 token；数据集权限实时查询 Python 数据库）====

  Scenario: body.dataset_ids 超出当前数据库授权范围时返回 403 且不执行 pipeline
  ...
  Scenario: 身份取自 token，dataset_ids 取当前数据库授权范围内的 body 子集
    Given Java access token 对应用户 sub=123 dataset_ids=[1,2] 有效
    And config_id 指向的 CHAT 模型对用户 123 可用
    When 前端携带该 token 调用 POST /api/v1/rag/stream body query="任意" dataset_ids=[1]
    Then HTTP 响应状态为 200
  ...
  Scenario: body 省略 dataset_ids 时按数据库当前全量授权范围召回
    Given Java access token 对应用户 sub=123 dataset_ids=[1,2] 有效
    And config_id 指向的 CHAT 模型对用户 123 可用
