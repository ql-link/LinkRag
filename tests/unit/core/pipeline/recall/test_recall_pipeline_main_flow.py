...
async def test_parallel_all_success():
    ...
    response = await pipeline.execute(RecallRequest(user_id=1, query="如何重试", dataset_ids=[10]))
    ...
    assert {hit.chunk_id for hit in response.candidate_hits} == {"c1", "c2", "c3", "c4", "c5"}
    assert [hit.chunk_id for hit in response.route_hits[SOURCE_DENSE]] == ["c1", "c2", "c3"]


@pytest.mark.asyncio
async def test_internal_candidate_pool_is_not_cut_by_public_top_k():
    dense = FakeRetriever(
        source=SOURCE_DENSE,
        hits=[_hit(f"c{i}", SOURCE_DENSE, 1.0 - i / 100) for i in range(5)],
    )
    ...
    assert len(response.candidate_hits) == 5
    assert len(response.route_hits[SOURCE_DENSE]) == 5
...
async def test_serial_fixed_order():
    ...
    dense = FakeRetriever(
        source=SOURCE_DENSE,
        hits=[_hit("c1", SOURCE_DENSE, 0.9)],
        delay_seconds=0.02,
        sequence_recorder=recorder,
    )
    sparse = FakeRetriever(
        source=SOURCE_SPARSE,
        hits=[_hit("c2", SOURCE_SPARSE, 5.0)],
        delay_seconds=0.02,
        sequence_recorder=recorder,
    )
    bm25 = FakeRetriever(
        source=SOURCE_BM25,
        hits=[_hit("c3", SOURCE_BM25, 12.0)],
        delay_seconds=0.02,
        sequence_recorder=recorder,
    )
