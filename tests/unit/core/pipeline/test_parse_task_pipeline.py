...
class TestParseTaskPipeline:
    async def test_execute_should_skip_when_task_record_missing(self):
        db = build_db(None)
        pipeline = ParseTaskPipeline(storage=MagicMock(), session_factory=lambda: db)
    ...
    async def test_execute_should_parse_upload_mark_success_and_chunk(
        ...
        pipeline = ParseTaskPipeline(storage=storage, session_factory=lambda: db)
        ...
        mock_chunk_markdown.assert_called_once_with("parsed content", "parsed/t-001.md")
        db.close.assert_called_once()
