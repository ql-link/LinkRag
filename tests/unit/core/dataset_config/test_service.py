...
async def test_recall_fusion_invalid_dataset_config_propagates():
    """dataset recall_config 中权重非法 → ValidationError 向上传播。"""
...
@pytest.mark.asyncio
async def test_recall_legacy_rrf_fields_are_ignored():
    """历史 JSON 中已废弃的 RRF 字段不阻断读取，也不会进入当前配置模型。"""
    db = _fake_db(
        row=_row(recall={"recall_fusion_strategy": "rrf", "rrf_k": 60})
    )

    bundle = await DatasetConfigService().get_config(user_id=1, dataset_id=2, db=db)

    assert not hasattr(bundle.recall, "recall_fusion_strategy")
    assert not hasattr(bundle.recall, "rrf_k")
