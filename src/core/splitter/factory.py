...
def _create_structured_chunking_engine(
    embedder: Any, config: "ChunkingConfig"
) -> ChunkingEngine:
    """按给定 ``ChunkingConfig`` 创建标准两阶段分片引擎。"""
    semantic_chunker = PercentileSemanticChunker(
        ...
        min_distance_gate=config.min_distance_gate,
    )
    chunker = StructuredSemanticChunker(
        semantic_chunker=semantic_chunker,
        heading_break_level=config.heading_break_level,
        min_candidate_chunk_tokens=config.min_candidate_chunk_tokens,
    )
...
def create_chunking_engine(config: "ChunkingConfig | None" = None) -> ChunkingEngine:
    ...
    ``config`` 为数据集级分块配置；``None`` 时取运行期系统 ``Settings`` 的 L1 基线
    （``ChunkingConfig.from_settings()``），保持未配置数据集行为与拆分前一致——含运维通过环境
    变量覆盖 ``CHUNKING_*`` 的场景。
    ...
    return _create_structured_chunking_engine(
        create_system_embedding_client(), config or ChunkingConfig.from_settings()
    )
...
def create_chunk_embedding_pipeline() -> ChunkEmbeddingPipeline:
    ...
    return ChunkEmbeddingPipeline(
        chunking_engine=_create_structured_chunking_engine(
            embedder, ChunkingConfig.from_settings()
        ),
        embedder=embedder,
