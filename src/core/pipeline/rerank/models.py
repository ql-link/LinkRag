...
- :class:`RerankRequest`：入参——query、user_id、融合后候选列表与可选 top_n。
- :class:`RerankedHit`：出参单项——在 ``RecallHit`` 元信息基础上补 rerank 分数与名次，
  保留融合解释信息（fused_score / 各路 scores）。
...
class RerankRequest:
    ...
    Attributes:
        ...
        hits: 融合后候选（``RecallHit``，按 fused_score 降序，不含正文）。
...
class RerankResponse:
    ...
    Attributes:
        query: 回显原始 query。
        hits: 重排后候选列表；rerank 生效时按 rerank_score 降序，降级时为当前融合顺序。
