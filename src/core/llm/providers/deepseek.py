        ...
        try:
            async with client.stream("POST", url, json=payload, headers=headers) as response:
                if response.status_code >= 400:
                    ...
                    if response.status_code == 401:
                        raise AuthenticationError(message="Invalid API Key", provider_type="deepseek")
                    if response.status_code == 429:
                        raise RateLimitError(message="Rate limit exceeded", provider_type="deepseek")
                    raise ProviderConnectionError(
                        message=f"DeepSeek API error: {response.status_code}",
                        provider_type="deepseek",
                    )
            ...
            raise ProviderConnectionError(message="Request timeout", provider_type="deepseek")
        except httpx.ConnectError:
            raise ProviderConnectionError(message="Connection failed", provider_type="deepseek")
