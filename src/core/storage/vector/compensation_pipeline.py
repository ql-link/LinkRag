...
from src.core.storage.qdrant import IndexedPoint, QdrantIndexStore
...
class VectorStorageCompensationPipeline(TransactionalPipelineMixin):
    ...
    async def reindex_failed_chunks(self, chunk_ids: Sequence[str]) -> ChunkIndexingResult:
        ...
        for record in record_map.values():
            ...
            try:
                ...
                point, embedding_model, sparse_vector = await self._build_reindex_point(
                    record, sparse_service=sparse_service
                )
    ...
    async def _resolve_sparse_vector_service(self, user_id: int) -> SparseVectorService:
        ...
        注入的 ``self.sparse_vector_service`` 一旦提供即对所有 user 生效（绕过解析），服务于
        测试与显式装配；生产补偿路径不注入，按 ``record.user_id`` per-record 解析，与本流程
        dense 重建（``aresolve_user_chunk_embedding_pipeline``）同源。
        """
