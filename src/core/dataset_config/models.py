...
class ChunkingConfig(BaseModel):
    """分块策略配置（8 项），消费点见 ``splitter/factory.py``。"""
    ...
    @field_validator("semantic_unit")
    @classmethod
    def _validate_semantic_unit(cls, v: str) -> str:
        normalized = v.strip().lower()
        if normalized not in {"sentence", "paragraph"}:
            raise ValueError("semantic_unit must be 'sentence' or 'paragraph'")
        return normalized
    ...
    def from_settings(cls) -> "ChunkingConfig":
        ...
        return cls(
            ...
            semantic_percentile=s.CHUNKING_SEMANTIC_PERCENTILE,
            semantic_unit=s.CHUNKING_SEMANTIC_UNIT,
            min_chunk_tokens=s.CHUNKING_MIN_CHUNK_TOKENS,
            max_chunk_tokens=s.CHUNKING_MAX_CHUNK_TOKENS,
            overlap_tokens=s.CHUNKING_OVERLAP_TOKENS,
            min_distance_gate=s.CHUNKING_MIN_DISTANCE_GATE,
        )
