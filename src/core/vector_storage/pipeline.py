...
class VectorStoragePipeline(TransactionalPipelineMixin):
    ...
    async def store_chunks(
        ...
        Args:
            request: 包含业务上下文的存储请求；其中的 ``chunks`` 字段在新边界下
                被忽略，索引依据来自已落库真值。
        ...
        # 旧调用方仍可能按 doc_id 触发；本期把"按 doc_id 反查"动作收敛进这里，
        # 不再走 ``list_vector_candidates_by_doc_id``——让 ``index_chunks`` 唯一对外。
        async with self.session_factory() as session:
            stmt = (
                select(self.repository.model_cls)
                .where(self.repository.model_cls.doc_id == request.doc_id)
                .where(
                    self.repository.model_cls.dense_vector_status.notin_(
                        CHUNK_DELETE_PROTECTED_STATUSES
                    )
                )
    ...
    async def index_chunks(
        ...
        *,
        ...
        和被替换的旧 ``index_document_chunks`` 比，本入口：

        * 不再按 ``doc_id`` 反查 SQL —— chunks 由 pipeline 现场过滤后透传。
        * SQL 多值 CAS 一条 UPDATE 覆盖 ``(PENDING, FAILED)`` 两种合法旧态，
          替代旧版 ``_group_records_by_dense_status`` 分组多次 UPDATE 的写法。
        * ``total_chunks`` 等于 ``len(chunks)``——不再做"全文档已完成数"折算。

        失败语义沿用旧版：某批 ``mark_indexing`` rowcount 不达预期或 embed/upsert
        失败时停止后续 batch；已完成 chunk 不会因后续失败而重做。

        Args:
            user_id / set_id / doc_id: 业务归属，仅日志可读用途；写入主键由 chunk
                自身携带。
            chunks: pipeline 现场过滤好的待处理 chunk 真值序列；每个元素需暴露
                ``chunk_id`` / ``user_id`` / ``set_id`` / ``doc_id`` /
                ``bucket_id`` / ``content`` / ``chunk_type`` / ``start_line`` /
                ``end_line`` / ``chunk_index`` / ``dense_vector_status`` 字段。
        """
        ...
        for batch_start in range(0, len(records), batch_size):
            ...
            # 同批后续 chunk 已有向量但不写入，保持 INDEXING 等待下次重试 / 补偿。
        ...
        *,
        ...
        Args:
            ...
            allowed_statuses: SQL 多值 CAS 的合法旧态集合；默认 ``(PENDING,)`` 服务于
                单 chunk 补偿路径，dense 入口 batch 路径会传 ``(PENDING, FAILED)``。
    ...
    async def _mark_sparse_indexing(
        ...
        *,
        ...
        ``allowed_statuses`` 默认 ``(PENDING,)`` 服务于单 chunk 补偿路径；
        sparse 文件级入口（``SparseIndexingPipeline._run_batch``）会传
        ``(PENDING, FAILED)`` 兼容首次 / retry 共用入口。
        """
