...
def upgrade() -> None:
    op.add_column(
        "kb_document_chunk",
        sa.Column("sparse_vector_status", sa.String(length=16), nullable=False, server_default="PENDING"),
    )
    op.add_column(
        "kb_document_chunk",
        sa.Column("sparse_vector_model", sa.String(length=128), nullable=True),
    )
    op.add_column(
        "kb_document_chunk",
        sa.Column("sparse_vector_nonzero_count", sa.Integer(), nullable=True),
    )
    op.add_column(
        "kb_document_chunk",
        sa.Column("sparse_vector_error_msg", sa.String(length=512), nullable=True),
    )
    op.add_column(
        "kb_document_chunk",
        sa.Column("sparse_vector_retry_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "kb_document_chunk",
        sa.Column("sparse_vector_last_retry_at", sa.DateTime(), nullable=True),
    )
