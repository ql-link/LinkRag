...
-- ===============================================
-- 本文件是「init.sql (0001 baseline) + 全部 Alembic migration」叠加后的
-- 当前表结构快照，仅用于人/工具查阅与代码评审参考。
-- ⚠️ 不是部署入口：
--   - 冷启动以 scripts/db/init.sql 为准（已 stamp 为 0001 baseline）；
...
-- 末次同步：migration 0006_20260522_drop_chunk_retry_and_error_fields
-- ===============================================
...
CREATE TABLE IF NOT EXISTS kb_document_chunk (
    ...
    lifecycle_status            VARCHAR(16) NOT NULL DEFAULT 'ACTIVE' COMMENT 'Chunk生命周期状态: ACTIVE/DELETING/DELETED/DELETE_FAILED',
