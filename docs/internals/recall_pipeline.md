...
├── fusion.py         # 固定 weighted score 粗融合
...
| 模型 | 方向 | 说明 |
| --- | --- | --- |
| `RecallRequest` | 入参 | `query` 必须非空；`user_id` 必须为正（HTTP 入口从凭证 claims 注入）；`dataset_ids` 允许空列表，表示不限数据集；`doc_ids` 可选；`top_k` 为正，表示融合候选池上限 / rerank 输入窗口，由数据集级 `recall_result_limit` 或系统 `RECALL_RESULT_LIMIT` 决定；`bm25_top_k` / `sparse_top_k` / `dense_top_k` 分别控制三路执行期召回深度；三路权重 override 来自数据集级 `recall_config`，不来自 HTTP 请求体 |
...
| `RecallPipelineConfig` | 装配配置 | `parallel`、`strict`、三路 `fusion_*_weight` |
...
## 7. 固定 weighted score 融合

融合逻辑在 `fusion.py`。系统和数据集均不再暴露算法切换开关；`fused_score` 表示归一化加权融合分，`RecallHit.scores` 始终保留各路原始分。
...
- 单项权重允许为 `0`；若本次 active source 权重和为 `0`，抛 `RecallValidationError`。
...
RecallPipelineConfig(
    parallel=True,
    strict=False,
    fusion_bm25_weight=0.15,
    fusion_sparse_weight=0.15,
    fusion_dense_weight=0.70,
)
```
...
| weighted score 融合 | `tests/unit/core/pipeline/recall/test_recall_pipeline_weighted_score.py` |
