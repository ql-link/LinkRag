...
Chunk 真值、Wiki 引用和外部索引的持久不一致由 Chunk/索引存储生命周期负责，Wiki 读取层不清理旧数据，也不按正文、行号或相似度猜测新旧 Chunk 映射。Wiki 只处理候选产生后、最终水合前不可避免的跨存储读取竞态：在配额与分页前，对“每知识库最多 50”的有界 BM25 ID 批量求当前 MySQL 真值交集，按库保序移除缺失 ID 后再合并，后项自然补位；候选池耗尽时允许 200 短页或空页。最终正文位置水合再次使用宽松内部入口保证 `results` 与 `chunks` 自洽；若标题预览在两次读取间失效，只做一次有界预览重算与新 ID 水合，仍失效则保留标题但降级为无预览，并由最终预览再次取得同 ID BM25 正文的所有权。正常过滤记录结构化事件；只有 SQL 成功但 ID 缺失才视为竞态，SQL、连接或解码失败记录异常堆栈并返回 500。

每个标题搜索结果只预览首个可见直属 Chunk，直属总数大于 1 时签发独立 `heading_chunks` 游标，从第二条引用开始每页最多 15 条。该游标按标题的规范资源身份签发，因此从跨数据集搜索结果进入单文档展开时仍可验签；它不读取子标题，也不推进顶层搜索游标。
...
搜索与标题展开读取 Chunk 位置时，仓储先在 MySQL 内按 `(chunk_id,doc_id)` 计算完整总数和稳定排名，再限制为前 10 个父标题 ID，随后才水合标题与父链。批量 Chunk 定位和文档整树不传位置上限，继续返回全部直接位置。标题直属 Chunk 查询把 `ACTIVE` 条件放在 keyset 与 `LIMIT` 同一条 SQL 中，并固定前瞻两页候选后宽松水合当前可见子集；读取竞态中失效的内部引用不占页面名额，前瞻不足时允许短页并记录 `wiki_heading_stale_refs_dropped`。客户端显式 `/chunk-locations` 仍使用严格全有或全无入口，任一 ID 不可见时整体 403。
