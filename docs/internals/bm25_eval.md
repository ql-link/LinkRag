# BM25 召回评测（Qdrant / Manticore）

记录 Qdrant 与 Manticore BM25 后端的召回质量评测。配套工具在 `scripts/dev/`，结论可复现。

> 评测全程只连本地 docker（Qdrant / Manticore），用独立 eval collection / table，跑完即删；
...
| [eval_bm25_recall.py](../../scripts/dev/eval_bm25_recall.py) | 召回评测：recall@k / nDCG@k / 后端 overlap@k；支持合成 / `--from-db` / `--from-beir` / `--with-manticore` 及质量门禁 |
...
| 数据集 | Qdrant R@10 / nDCG | Manticore R@10 / nDCG | Manticore↔Qdrant overlap@10 |
| --- | --- | --- | --- |
| NFCorpus（英文） | 0.156 / 0.323 | 0.156 / 0.323 | 0.934 |
| CovidRetrieval（中文） | 0.914 / 0.815 | 0.919 / 0.818 | 0.861 |

1. **实现是标准 BM25**：NFCorpus nDCG@10=0.323 与 BEIR 公布的 BM25 基线（≈0.325）几乎
   吻合，侧面交叉验证打分逻辑没写歪。
2. **fine 路有效**：合成 / 中文集上"只在 fine 段命中（query 词嵌在长词里被细分出，如
   query『网络』命中文档的『无线网络』）"的文档能被召回，纯 coarse 召不到。
3. **Manticore 采用 coarse-only**：早期 coarse+fine BM25F 在 CovidRetrieval 上只有
...
```bash
# 中文集：hf-mirror 下载 + 转 BEIR，再评测
python scripts/dev/fetch_cmteb_retrieval.py --dataset CovidRetrieval --sample 10000 --out /tmp/covid_beir
python scripts/dev/eval_bm25_recall.py --from-beir /tmp/covid_beir --k 10 --with-manticore
