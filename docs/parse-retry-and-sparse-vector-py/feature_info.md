...
- **brief 版本历史**：
  ...
  - v3：2026-05-26（状态权威单源化：删除 log.task_status / log.failure_reason；删除 pipeline.chunk_count / retry_count / last_retry_at；新增 pipeline.parsing_status / parsing_duration_ms；pipeline 表升格为"文件解析流程状态表"）
- **产物清单**：
  - `brief.md` — v3 草稿（待冻结）
  - `acceptance.feature` — 基于 v2 的草稿，待 v3 brief 冻结后再次整体重写
- **v3 主要修订**：
...
- **下一步**：
  ...
  2. 审阅 v3 brief 并确认冻结
  3. 冻结后重新进入 `acceptance-generator` 基于 v3 整体重写 `acceptance.feature`
