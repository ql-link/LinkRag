...
RAG 问答在 Python 端（`/api/v1/rag/stream`）流式生成结束后，发送一条 `ChatTurnMessage`，由 **Java 消费并落库**：在单事务里写入 `chat_message` 一行（一行一轮：query + answer 同行），并更新 `chat_conversation` 的 `last_config_id` / `last_model_name` / `updated_at`。Python 侧不写这两张表。
...
| `request_id` | string | ✅ | 请求追踪 ID / 幂等键；Java 据此去重，写入 `chat_message.request_id` |
| `user_id` | int | ✅ | 用户 ID |
| `query` | string | ✅ | 用户提问 → `chat_message.query` |
| `answer` | string | ✅ | LLM 回答 → `chat_message.answer`（`partial` 为半截，`failed` 可空串） |
| `config_id` | int | ✅ | 本轮所用 LLM 配置 ID → `chat_message.config_id` |
| `model_name` | string | ✅ | 模型名快照（可空时为空串）→ `chat_message.model_name` |
...
> token 与 `provider_type` / `latency_ms` 已从本消息**移除**（LINK-191），改由统一用量消息承载。
