...
- **删除一致性**：Chunk 删除时需清理 Qdrant、ES 等外部索引，并把 MySQL `kb_document_chunk.lifecycle_status` 从 `ACTIVE` 推进到 `DELETING/DELETED/DELETE_FAILED`。`dense_vector_status` 只保留 `PENDING/SUCCESS/FAILED` 稠密向量产物结果，不再承载删除生命周期。
