...
- 必须从产品和开发两个视角同时分析：产品视角关注问题、用户、目标、范围、非目标；开发视角关注模块、状态、数据、依赖、幂等、补偿和排障。
- 项目上下文只写会影响需求判断、边界收敛或后续设计的内容，不输出泛泛的项目现状或代码观察记录。
...
不要把“读到了哪些目录/文件”直接写成独立章节；只把它们转化为对需求边界、模块关系、流程风险、复用约束有用的结论。
...
- **本次可能包含：** [当前已经能判断的范围]
- **本次明确不做：** [用户已明确排除，或基于当前阶段不建议纳入的内容]
- **范围待确认：** [会影响 PRD 范围的问题]
...
本章节只写与需求决策直接相关的模块、流程、约束和复用依据。不要输出泛泛的“已观察到项目现状”。

### 3.1 可能涉及的模块

| 模块/能力 | 当前作用 | 与本需求的关系 | 依据 | 判断状态 |
| :--- | :--- | :--- | :--- | :--- |
| [模块 A] | [当前作用] | [复用/修改/新增依赖/待确认] | [来自用户输入/项目文档/代码线索/推测] | [已确认/推测/待确认] |
...
- **必须遵守的既有约束：** [状态机、公共契约、配置、权限、数据一致性、外部依赖等]
- **可复用的项目能力：** [已有服务、任务、组件、消息、存储、工具等]
- **不能直接复用或需要改造的点：** [原因和影响]
- **待确认的项目假设：** [如果假设不成立，会影响哪些流程或范围]

## 4. 全链路推演

### 4.1 完整过程描述

[用连续叙述说明从触发、前置条件、系统处理、状态变化、异常处理到最终反馈的完整过程。不要只写步骤清单；最终收敛后，这一节应能直接作为 PRD 的主流程输入。]
...
| 编号 | 问题 | 为什么必须确认 | 不确认的影响 | 阻塞级别 | 建议确认人 | 当前状态 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Q1 | [问题 A] | [原因] | [影响] | 阻塞 PRD / 影响技术设计 / 可后置 | [用户/产品/研发/架构/运维] | 待提问 / 已提问 / 已确认 / 暂缓 |
...
- 需求目标、范围、非目标必须服务于后续 PRD 边界判断；如果无法判断，应进入“待确认问题”，不要用空泛目标填充。
- 全链路推演必须包含一个 Mermaid 流程图；流程图只画主链路和关键异常分支，不要把所有细节都塞进图里。
- `项目上下文` 不得包含“已观察到的项目现状”这类观察记录章节；只输出模块关系、流程关系、关键约束和复用依据。
...
- 待确认问题必须标注阻塞级别；下一轮优先提问应优先选择“阻塞 PRD”的问题。
