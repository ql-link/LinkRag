...
- 速览摘要——必要内容要写透
- 实现方案——不展开处理步骤、不做技术选型、不写物理 schema（字段类型/索引/迁移）、不写接口字段/类名/方法签名，这些归 `technical-design`
- PRD 散文——不写产品营销话术、不写"用户体验"等无法验证的说辞

> 关于数据结构：brief **可以涉及数据结构设计，但只到概念级**——关键实体、核心字段、实体关系、新增/改了哪些数据对象（这是界定需求范围与可行性的一部分）。物理 schema（类型、长度、索引、约束、默认值、迁移）归 `technical-design` + alembic。
...
- **数据结构分层**：第 3 章可写**概念数据模型**（关键实体 / 核心字段 / 关系），但不写**物理 schema**（类型、长度、索引、约束、默认值、迁移）——后者归 `technical-design` + alembic。
...
- **关键数据结构（概念级）**：本模块新增 / 改动的关键实体、核心字段、实体关系（只到"有哪些数据、谁关联谁"，**不写类型 / 索引 / 约束 / 迁移** —— 那些归 `technical-design`）
...
- 业务流程能在脑子里跑通，含关键异常
- 每个模块都有"位置 + 职责 + 复用/新增 + 触碰契约 + 关键数据结构（概念级）+ 不确定性"
...
- 越界写了物理 schema（字段类型/索引/迁移）、类名、方法签名等 how（L2 最小实现思路附录除外，且也不到物理 schema）
- 风险表用泛泛措辞
...
- **车道差异**：L3 的第 3 章只到影响面（how 全由 TD 承接）；L2 跳过 `technical-design`，第 3 章含"最小实现思路"附录，冻结后直接进编码
- **冻结后**：转入 `acceptance-generator` 生成 Gherkin 验收契约
- **不允许**：未经冻结直接跳到 `technical-design`；在 brief 阶段写物理 schema 等代码层 how（概念数据模型与 L2 最小实现思路除外）
