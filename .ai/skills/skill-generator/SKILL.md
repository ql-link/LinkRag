...
description: 当用户提出想根据某个需求、工作流、角色职责或项目约定创建一个全新的 Codex/Agent skill 时使用；先深度理解用户目标、触发场景、输入输出、执行边界和质量标准，在需求不清时主动追问，最终生成结构清晰、触发准确、流程可执行、可维护的 `.ai/skills/<skill-name>/SKILL.md`。
...
本 skill 用于把用户的原始想法、工作流、项目规范或重复任务，沉淀为一个新的 Agent/Codex skill。
